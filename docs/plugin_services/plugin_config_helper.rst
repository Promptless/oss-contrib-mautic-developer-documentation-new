...
``getBundleConfig(string $bundleName, string $configKey = '', bool $includePlugins = false)`` returns the requested section of a bundle or Plugin's configuration. ``BundleHelper`` also exposes ``getMauticBundles()`` and ``getPluginBundles()`` to list the registered bundles and Plugins.
