...
**PUT** creates a Marketing Message if the given ID does not exist and clears all the Marketing Message information, adds the information from the request.
**PATCH** fails if the Marketing Message with the given ID does not exist and updates the Marketing Message field values with the values form the request.
...
To edit a Marketing Message and return a 404 if the Marketing Message is not found:

``PATCH /messages/ID/edit``

To edit a Marketing Message and create a new one if the Marketing Message is not found:
