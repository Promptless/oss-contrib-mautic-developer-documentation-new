...
Mautic ships many helper classes. A few of the most commonly used ones are highlighted below.
...
Use ``Mautic\CoreBundle\Helper\DateTimeHelper`` to convert between UTC and the local timezone:
...
Use ``Mautic\CoreBundle\Helper\InputHelper`` to clean user input:
